...
namespace Dashboard.API.Controllers
{
    ...
    public class AccountController : BaseController
    {
        ...
        public async Task<IActionResult> RestartPasswordAsync([FromBody] ResetPasswordVM model)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            }
            var response = await _accountService.RestartPasswordAsync(model.Email, model.Token, model.Password);
            return GetResult(response);
        }
    }
}
