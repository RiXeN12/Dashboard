...
namespace Dashboard.BLL.Services.UserService
{
    public class UserService : IUserService
    {
        ...
        public async Task<ServiceResponse> UpdateAsync(CreateUpdateUserVM model)
        {
            ...
            {
                var currentRole = user.UserRoles.First().Role.Name;
                result = await _userRepository.RemoveFromRoleAsync(user, currentRole);

                if (!result.Succeeded)
                {
                    return ServiceResponse.BadRequestResponse($"Помилка при видаленні ролі: {result.Errors.First().Description}");
                }
                ...
                if (!result.Succeeded)
                {
                    return ServiceResponse.BadRequestResponse($"Помилка при додаванні нової ролі: {result.Errors.First().Description}");
                }
            }
